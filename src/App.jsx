...
export default function App() {
  ...
  async function handleSearch(query) {
    ...
    try {
      ...
      if (res && res.length) setMatchedCourses((prev) => [...res, ...prev]);
      else {
        setMatchedCourses((prev) => [
          {
            id: `mock-${Date.now()}`,
            title: `${query} (sample match)`,
            platform: 'Curata Labs',
            duration: '6 months',
            fee: '₹45,000',
            notes: 'End-to-end ML + production engineering',
          },
          ...prev,
        ]);
      }
